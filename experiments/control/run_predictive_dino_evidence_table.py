"""Build an evidence table for DINO preservation predictive-validity runs."""
...
METRICS = [
    ("rho_vs_dino_spearman", "rho vs DINO-preservation Spearman", "negative"),
    ("dino_beta_rho", "rho beta for DINO preservation", "negative"),
    (
        "matched_pair_dino_low_minus_high",
        "low-risk minus high-risk DINO preservation",
        "positive",
    ),
]
...
def row_from_summary(label: str, summary: dict[str, Any],
    ...
    values = []
    for seed_row in summary.get("seed_rows", []):
...
def readiness_label(pass_count: int, total: int) -> str:
    if pass_count == total:
        return "dino_preservation_predictive_ready"
    if pass_count >= total - 2:
        return "dino_preservation_predictive_borderline"
    return "dino_preservation_predictive_sensitive"
...
def write_markdown(path: Path, rows: list[dict[str, Any]]) -> None:
    lines = [
        "| Summary | Metric | Mean | 95% bootstrap CI | Wins | p(sign) | Pass |",
        "| --- | --- | ---: | ---: | ---: | ---: | --- |",
    ]
    for row in rows:
        ...
        lines.append(
            "| {summary} | {metric} | {mean} | {ci} | {wins}/{n} | {p} | {passed} |".format(
                summary=row["summary"],
                metric=row["display"],
...
def main() -> None:
    ...
    ap.add_argument("--seed", type=int, default=4213)
    ap.add_argument("--protocol", default=None)
    ap.add_argument("--protocol-key", default="predictive_dino_evidence_table")
    args = ap.parse_args()
    ...
    for label, path in [parse_summary_arg(raw) for raw in args.summaries]:
        ...
        missing = [
            ...
            if key not in summary.get("seed_level", {})
        ]
        if missing:
            raise KeyError(f"{label} is missing DINO metrics: {missing}")
        for key, display, expected in METRICS:
            rows.append(
                row_from_summary(
                    label,
                    summary,
    ...
    result = {
        ...
        "_meta": run_metadata(seed=args.seed, extra={
            "script": "experiments/control/run_predictive_dino_evidence_table.py",
    ...
    table_path = out.parent / "predictive_dino_evidence_table.md"
