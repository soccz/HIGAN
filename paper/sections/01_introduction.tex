...
we extend this picture to the \emph{second-fundamental form} and show
that the resulting curvature signature is what distinguishes
topology-changing edits from texture-only edits, on both StyleGAN and
Stable Diffusion.
...
Two headline empirical findings:
...
$r = +0.99$ (bedroom) and $r = +1.00$ Spearman (FFHQ) across
attributes. \textbf{(b)} Across 13 attribute--domain combinations,
the layer-localisation IoU score is positive at every top-$k$
