"""Generate locked protocols for prompt-ensemble follow-up stress tests."""
...
def add_predictive_block(experiments: list[dict[str, Any]], *,
                         ...
                         seeds: list[int], overrides: dict[str, Any]) -> None:
        ...
        for seed in seeds:
            ...
            experiments.append({
                ...
                "claim_tested": (
                    ...
                    f"directionally stable under {label} stress in {domain}."
                ),
    ...
    for domain in ["church", "bedroom"]:
        ...
        experiments.append({
            ...
            "claim_tested": (
                f"Aggregate prompt-ensemble {label} predictive-validity runs "
                f"for {domain}."
            ),
            "args": {
                "inputs": inputs,
...
def v54_prompt_ensemble_followup() -> dict[str, Any]:
    version = "control_v54_prompt_ensemble_followup_predictive_validity"
    experiments: list[dict[str, Any]] = []
    add_predictive_block(
        experiments,
        version=version,
        label="high_ntest128",
        out_root="experiments/out/control_cross_domain_risk_predictive_prompt_ensemble_high_ntest128",
        seeds=list(range(2082, 2087)),
        overrides={"n-test": 128},
    )
    add_predictive_block(
        experiments,
        version=version,
        label="strict_gain_match",
        out_root="experiments/out/control_cross_domain_risk_predictive_prompt_ensemble_strict_gain_match",
        seeds=list(range(2087, 2092)),
    ...
    return {
        ...
        "purpose": (
            "Stress-test prompt-ensemble semantic directions under higher "
            "test-set size and stricter semantic gain matching."
        ),
        "experiments": experiments,
    }


def v55_evidence() -> dict[str, Any]:
    summaries = [
    ...
    return {
        "version": "control_v55_predictive_stress_evidence_table_ensemble_followup",
        "locked_on": "2026-05-28",
        "purpose": (
            "Build the extended predictive stress evidence table including "
            "prompt-ensemble high-n-test and strict-gain follow-ups."
        ),
        "experiments": [
            {
                "key": "predictive_stress_evidence_table_v3",
                "order": 1,
                "script": PY_STRESS_EVIDENCE,
                "claim_tested": (
                    "Prompt-ensemble semantic directions should clarify which "
                    "sensitivity failures remain after prompt averaging."
                ),
                "args": {
                    "summaries": summaries,
                    "out": "experiments/out/control_predictive_stress_evidence_table_v3",
                    "seed": 4211,
                },
            }
        ],
...
def v56_audit() -> dict[str, Any]:
    ...
    return {
        "version": "control_v56_main_grade_audit_v13",
        "locked_on": "2026-05-28",
        "purpose": (
        ...
        "experiments": [
            {
                "key": "main_grade_audit_v13",
                "order": 1,
                "script": PY_AUDIT,
                "claim_tested": (
                    "All locked control protocols through prompt-ensemble "
                    "follow-up stress tests have complete traceable outputs."
                ),
                "args": {
                    "protocols": protocols,
                    "out": "experiments/out/control_campaign_audit_main_grade_v13",
                    "seed": 3722,
                },
            }
        ],
...
def main() -> None:
    write_protocol(
        ...
        v54_prompt_ensemble_followup(),
    )
    write_protocol(
        "control_v55_predictive_stress_evidence_table_ensemble_followup.json",
        v55_evidence(),
    )
    write_protocol("control_v56_main_grade_audit_v13.json", v56_audit())
